...
namespace LMS.API.Controllers
{
    public partial class CourseController
    {
        ...
        public async Task<IActionResult> GetTaskComments(Guid courseId,Guid taskId)
        {
            ...
            //comments = task.Comments.Adapt<List<TaskCommentDTO>>();
        ...
        public async Task<IActionResult> AddTaskComments(Guid courseId, Guid taskId, [FromBody] CreateTaskCommentDTO createTaskCommentDTO)
        {
            var task = await _applicationDbContext.Tasks.FirstOrDefaultAsync(t => t.Id == taskId && t.CourseId == courseId);
            if (task == null)
                return NotFound();
